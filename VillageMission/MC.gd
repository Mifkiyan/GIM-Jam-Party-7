...
func _on_InteractArea_area_entered(area):
	if area.is_in_group("Pintu rumah 1"):
		var d = Dialogic.start("1")
		d.pause_mode = PAUSE_MODE_PROCESS
		get_parent().call_deferred('add_child', d)
		d.connect("timeline_end",self,"after_dialog")
		get_tree().paused = true
		
	if area.is_in_group("Pintu rumah 2"):
		var d = Dialogic.start("2")
		d.pause_mode = PAUSE_MODE_PROCESS
		get_parent().call_deferred('add_child', d)
		d.connect("timeline_end",self,"after_dialog")
		get_tree().paused = true
		
	if area.is_in_group("Pintu rumah 3"):
		var d = Dialogic.start("3")
		d.pause_mode = PAUSE_MODE_PROCESS
		get_parent().call_deferred('add_child', d)
		d.connect("timeline_end",self,"after_dialog")
		get_tree().paused = true
		
	if area.is_in_group("Pintu rumah 4"):
		var d = Dialogic.start("4")
		d.pause_mode = PAUSE_MODE_PROCESS
		get_parent().call_deferred('add_child', d)
		d.connect("timeline_end",self,"after_dialog")
		get_tree().paused = true
		
	if area.is_in_group("Pintu rumah 5"):
		var d = Dialogic.start("5")
		d.pause_mode = PAUSE_MODE_PROCESS
		get_parent().call_deferred('add_child', d)
		d.connect("timeline_end",self,"after_dialog")
		get_tree().paused = true
